...
export const projectDetails: ProjectDetail[] = [
  {
    ...
    tagline:
      'A URL shortener is the "hello world" of backend — but the scalable version is anything but. It\'s read-heavy (every redirect is a lookup), needs unique IDs without coordination, has to absorb…',
    problem:
      'A URL shortener is the "hello world" of backend — but the scalable version is anything but. It\'s read-heavy (every redirect is a lookup), needs unique IDs without coordination, has to absorb bursty click traffic, and must not fall over when a link goes viral. That makes it the perfect first rung.',
    whatItDoes: [
  ...
  },
  {
    ...
    tagline:
      'A broadcast server looks trivial: "a client subscribes to a topic, and every message published to that topic is sent to every subscriber." The trap is what real-time and at scale do to that…',
    problem:
      'A broadcast server looks trivial: "a client subscribes to a topic, and every message published to that topic is sent to every subscriber." The trap is what real-time and at scale do to that sentence. WebSockets are long-lived and stateful, so the server now holds thousands of open sockets at once. Some of those clients read slowly — and a single slow reader must not be allowed to stall the fast ones or balloon the server\'s memory (backpressure). And the moment you run more than one server instance, a message…',
    whatItDoes: [
  ...
  },
  {
    ...
    tagline:
      "A single-node cache is a HashMap with an eviction rule. The moment one box can't hold the working set — or can't survive being rebooted — you need a cluster, and every easy thing gets hard: which…",
    problem:
  ...
  },
  {
    ...
    tagline:
      "A message broker looks like a queue with extra steps — until you ask it to never lose a committed message, never reorder within a key, hand the same stream to many independent consumers, and do it…",
    problem:
      "A message broker looks like a queue with extra steps — until you ask it to never lose a committed message, never reorder within a key, hand the same stream to many independent consumers, and do it while a producer is hammering it at hundreds of MB/s. Kafka's answer to all of that is one deceptively simple idea: **an append-only log on disk, split into segments, split into partitions, read by cursor.** This project builds that log from scratch. It's Tier 4 because the hard parts — durable sequential writes,…",
    whatItDoes: [
  ...
  },
  {
    ...
    tagline:
      'A replicated key-value store sounds like "a HashMap, but on three machines." The trap is the word replicated: the moment more than one node can accept a write, you have to answer "which write…',
    problem:
  ...
  },
  {
    ...
    tagline:
      '"Just forward the request" is a lie you tell yourself until the first 2 GiB upload buffers into RAM, a client smuggles a Transfer-Encoding header past you to the backend, one slow upstream drags…',
    problem:
  ...
  },
  {
    ...
    tagline:
      "Project 11 turned one video file into a playable HLS/DASH stream — but it assumed the file was already at the right codec and bitrates. Real platforms don't get that gift: an upload arrives as one…",
    problem:
      "Project 11 turned one video file into a playable HLS/DASH stream — but it assumed the file was already at the right codec and bitrates. Real platforms don't get that gift: an upload arrives as one giant 4K ProRes (or a phone's H.265), and before it can be packaged it has to be transcoded into the whole ladder (1080p, 720p, 480p, …). Transcoding a two-hour movie serially on one machine takes hours — longer than the movie. The only way out is to go wide: cut the source into chunks, transcode the chunks in…",
    whatItDoes: [
  ...
  },
  {
    ...
    tagline:
      "Project 11 packaged a file that already existed; project 12 produced that file from an upload. This one has no file — the media is arriving, right now, from a camera. A broadcaster (OBS, ffmpeg, a…",
    problem:
      'Project 11 packaged a file that already existed; project 12 produced that file from an upload. This one has no file — the media is arriving, right now, from a camera. A broadcaster (OBS, ffmpeg, a phone) opens a socket and starts pushing H.264 + AAC over RTMP, and thousands of viewers want to watch **within a few seconds of real life**. That "few seconds" is the whole game: regular HLS cuts 6-second segments and makes a player buffer three of them, so the viewer is ~15–30 seconds behind the glass. Low-Latency…',
    whatItDoes: [
  ...
  },
  {
    ...
    tagline:
      "Projects 11–13 always had HTTP to hide behind: TCP guaranteed the bytes arrived, in order, eventually, and a player could buffer its way over any bump. This one takes that safety net away.…",
    problem:
      "Projects 11–13 always had HTTP to hide behind: TCP guaranteed the bytes arrived, in order, eventually, and a player could buffer its way over any bump. This one takes that safety net away. Real-time media — a video call, a game stream, an esports feed — cannot wait for TCP's retransmit-and-reorder, because a packet that arrives 400 ms late is useless: the moment it was meant to be shown has already passed. So real-time media runs on UDP, which gives you exactly nothing — packets are dropped, duplicated,…",
    whatItDoes: [
  ...
  },
  {
    ...
    tagline:
      "This is the capstone — the marquee project. There's no new primitive to invent here; the hard part is integration: wiring the pieces you already built (chat fan-out from 03, HLS packaging from 11,…",
    problem:
      "This is the capstone — the marquee project. There's no new primitive to invent here; the hard part is integration: wiring the pieces you already built (chat fan-out from 03, HLS packaging from 11, transcode from 12, RTMP ingest from 13) into one glass-to-glass pipeline that survives a real crowd. A broadcaster's frame is captured, ingested, transcoded into an ABR ladder, packaged as low-latency HLS, delivered through an edge, and painted on a thousand viewers' screens — while chat scrolls in real time. Then a…",
    whatItDoes: [
  ...
  },
  {
    ...
    tagline:
      "Project 15 gave you one SFU: a server that forwards a publisher's encoded RTP to many subscribers in the same room, no transcode, one upload → one tailored download per viewer. It makes a…",
    problem:
      "Project 15 gave you one SFU: a server that forwards a publisher's encoded RTP to many subscribers in the same room, no transcode, one upload → one tailored download per viewer. It makes a 50-person call in one data centre possible. This capstone asks the question that turns a regional SFU into a global conferencing system: what happens when the 50 people are in Tokyo, Frankfurt and São Paulo at once? The naive answer — point everyone at a single SFU — fails twice. First on latency: a Tokyo viewer watching a…",
    whatItDoes: [
  ...
  },
  {
    ...
    tagline:
      "Moving money is the one place in backend where a race condition has a dollar figure attached. A ledger looks like CRUD — insert a row, read a balance — until two requests touch the same account at…",
    problem:
  ...
  },
  {
    ...
    tagline:
      "Downloading a file over HTTP is trivial: one server, one connection, done. BitTorrent throws all of that away. There is no server — you assemble the file out of blocks handed to you by anonymous…",
    problem:
  ...
  },
  {
    ...
    tagline:
      'A workflow engine sells one promise: durable execution. You write a normal-looking function — "charge the card, wait 3 days, if not cancelled ship the order, email the customer" — and the engine…',
    problem:
      'A workflow engine sells one promise: durable execution. You write a normal-looking function — "charge the card, wait 3 days, if not cancelled ship the order, email the customer" — and the engine guarantees it runs *to completion, exactly as written, even though the process running it will crash, deploy, and restart many times before it finishes.* That is a wild promise, and the only way to keep it is to stop storing the program\'s state and start storing its history: an append-only log of everything that…',
    whatItDoes: [
