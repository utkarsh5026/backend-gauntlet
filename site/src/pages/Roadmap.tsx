    ...
    <div className="space-y-12">
      <header className="max-w-2xl space-y-3">
        <p className="m-0 text-[0.85rem] text-fg-muted">
          <span className="text-accent">$</span> ls projects/ · 22 projects, 7 tiers, easy → hard
        </p>
        <h1 className="font-display m-0 text-xl font-bold tracking-tight sm:text-2xl">projects</h1>
        <p className="m-0 text-fg-muted">
          Click any project for the problem, verticals, and SPEC links. Progress is a snapshot from{" "}
          <code className="text-fg">make status</code>.
        </p>
      </header>
      ...
      <div className="space-y-10">
        {tiers.map((tier) => (
          <section key={tier.id}>
            ...
            <ul className="m-0 mt-5 list-none divide-y divide-line overflow-hidden rounded-md border border-line p-0">
              {tier.projects.map((p) => (
                <li key={p.id}>
                  <Link
                  ...
                  >
                    <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                      ...
                      <span className="ml-auto text-[0.8rem] text-fg-muted">{p.progress}%</span>
                    </div>
