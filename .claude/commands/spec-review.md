...
2. Verify it passes the gate and check what's actually implemented vs still
   `raise NotImplementedError`: `make verify` from the project dir.
3. Review against the SPEC, organized by:
   ...
   - **Idiomatic Python** — typing (pyright strict), exception design, blocking calls
     on the event loop, unnecessary copies/allocations on hot paths.
   - **Acceptance criteria & Proof** — walk each vertical's "Done when ALL true"
     ...
     the definition-of-done gate too (bench numbers, design doc, `make verify` green).
