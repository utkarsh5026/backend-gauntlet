...
so you get one source of truth with colors, emojis and readable output.
...
def wait_db() -> None:
    ...
    for _ in range(30):
        probe = subprocess.run(
            [*COMPOSE, "exec", "-T", "postgres", "pg_isready", "-U", "shortener"],
            cwd=str(PROJECT_DIR),
...
def bench() -> None:
    step("📊", "running id_gen criterion bench…")
    cargo("bench", "-p", CRATE, "--bench", "id_gen")


@task("bench-seed", "🌱", "Bench", "Seed N bench links into Postgres (N=50000 ...)")
def bench_seed() -> None:
    require("node", "Install Node.js to run the bench harness.")
    step("🌱", "seeding bench links…")
    run(["node", "bench/seed.js"], cwd=PROJECT_DIR, env=load_dotenv())


@task("bench-smoke", "🔥", "Bench", "Node redirect sanity check (server must run)")
def bench_smoke() -> None:
    require("node", "Install Node.js to run the bench harness.")
    run(["node", "bench/smoke.js"], cwd=PROJECT_DIR, env=load_dotenv())


@task("bench-load", "🏋️", "Bench", "k6 redirect load test: seed + all scenarios")
def bench_load() -> None:
    require("node", "Install Node.js to run the bench harness.")
    step("🏋️", "running k6 load scenarios…")
