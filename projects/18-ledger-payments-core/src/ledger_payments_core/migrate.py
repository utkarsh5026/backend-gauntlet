"""`python -m ledger_payments_core.migrate` — apply migrations and exit.

The Python answer to `sqlx migrate run`, wired into `make migrate`. Reads the same
`.env` the server does, so there is one source of truth for `DATABASE_URL`.
"""
...
async def _run(directory: Path) -> int:
    config = Settings()
    pool = await create_pool(config.database_url, min_size=1, max_size=2)
    try:
