...
Raw asyncpg with parameterized SQL, not an ORM — see `store.py` for why this
project's lessons live in the statements themselves. Every query is a string you
can read, with `$1`-style placeholders sent to the server separately from the data,
so there is no string-built SQL and no injection surface.
"""
...
async def _init_connection(conn: asyncpg.Connection[asyncpg.Record]) -> None:
    ...
    `jobs.ladder` and `tasks.kind` are JSONB. Without this codec asyncpg returns
    them as the raw text they arrived as, and the first caller that forgets to
    `json.loads` writes a JSON-encoded *string* back into the column on its next
    update — a `kind` that no longer decodes, and a task no worker can run.
    """
...
async def create_pool(dsn: str, *, min_size: int, max_size: int) -> asyncpg.Pool[asyncpg.Record]:
    ...
    Postgres side, so `max_size` × worker processes has to stay under the server's
    `max_connections` — and a worker pool that scales out for the boss fight moves
    that product. An unbounded pool turns "add workers to go faster" into "too many
    clients already".
    """
