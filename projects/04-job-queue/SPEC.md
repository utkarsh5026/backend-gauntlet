...
**Done when ALL true:**
- [ ] `enqueue` inserts; `claim` selects **and** locks in one atomic statement (`FOR UPDATE SKIP LOCKED`).
- [ ] **Two+ workers never claim the same row** — a second worker steps over locked rows instead of blocking on them.
- [ ] `claim` takes a **batch** (`LIMIT n`) in one round-trip, and respects `run_at <= now()`, oldest-first, and the per-queue selector.
- [ ] A **partial index over the `ready` rows** keeps the claim `SELECT` cheap as the table grows — shown with-vs-without in the bench.
...
**Done when ALL true:**
- [ ] A claim stamps `locked_by` / `locked_until` and flips the job to `running`; a successful worker **acks** it to `done`.
- [ ] A **reaper** returns `running` jobs whose lease has expired to `ready` — a crashed worker loses nothing.
- [ ] A worker killed mid-job has its job **picked up by another** once the lease expires (chaos-tested).
- [ ] The at-least-once consequence is faced explicitly: handlers are **idempotent** (or use an idempotency key), documented in `docs/04-design.md`.
...
**Done when ALL true:**
- [ ] On failure: `attempts` is incremented, `last_error` recorded, and if attempts remain the job is rescheduled (`run_at = now() + backoff`, state → `ready`).
- [ ] Backoff is **exponential with jitter, capped** at a maximum — not fixed, not bare `2^n`.
- [ ] A **poison message** (fails every time) lands in the **DLQ** and **stops** — it never loops forever.
- [ ] The DLQ is **inspectable and requeueable** — a dead job you can't see or requeue is silent data loss.
...
- [ ] A **slow poll fallback** remains, so a missed `NOTIFY` or a future `run_at` is never stranded forever.
...
### Protocols / API
- [ ] A small typed JSON API: `POST /jobs` to enqueue (validate `queue`/`kind`,
  cap payload size), `GET /jobs/{id}` for status, and a way to **list the
  DLQ** and **requeue** a dead job.
- [ ] Sensible status codes (`201` on enqueue, `404` for an unknown id, `400` for
...
- [ ] Postgres is the durable source of truth; a job survives a full restart of
  ...
  hub).
- [ ] The claim is atomic (V1) and the lease is honoured (V2): assert, with a
...
- [ ] Authenticate the enqueue API (an API key / token) — an open `POST /jobs` is
  an open door to make your workers do arbitrary work.
- [ ] Validate and **cap** everything the caller controls: payload size, `queue`/
...
### Observability
- [ ] Gauges: queue depth (`ready`), in-flight (`running`), DLQ size, oldest
  ready job's age (**the** lag metric — if this climbs, you're falling behind).
- [ ] Counters: jobs enqueued / completed / retried / dead-lettered, leases
  reaped (a non-zero reap rate means workers are dying or leases are too
  short), claims that came back empty.
- [ ] Histograms: job execution time and end-to-end latency (enqueue → done) p50/
  p99. A `tracing` span per job carrying `job.id`, `kind`, and `attempt`.
